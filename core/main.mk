...
ifneq (,$(user_variant))
  ...
  ifeq ($(user_variant),userdebug)
    ...
    # FIXME: enable debug feature for user build for automation test
    enable_target_debugging := true
  endif
  ...
  ifneq (true,$(DISABLE_DEXPREOPT))
    ifeq ($(user_variant),user)
      ifeq ($(HOST_OS),linux)
        WITH_DEXPREOPT := false  # note, when enable this in user release, then must enlarge the BOARD_SYSTEMIMAGE_PARTITION_SIZE in BoardConfig.mk(maybe also need adjust partition mbn)
      endif
    endif
  endif
...
#qrd extension update package
qrdext_update:droidcore
droid:qrdext_update
